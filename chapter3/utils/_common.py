"""第 3 章 ハンズオン用ユーティリティの共通モジュール。

setup.py / setup_cicd.py / delete_resources.py から利用する。
...
def update_env_file(env_path: Path, values: dict[str, str]) -> None:
    """`.env` ファイルのキーを追加・更新する（既存の他キーは保持）。

    dbt の `profiles.yml` が `env_var('S3_BUCKET_ATHENA')` などで参照する値を、
    OS に依存しない形でローカルに保存するために使う。`uv run --env-file` や
    `gh secret set --env-file` がこのファイルを読み込む。
    """
    ...
    env_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"  {env_path} を更新しました ({', '.join(values)})")


def print_env_export_hint(env_key: str, env_value: str) -> None:
    """dbt を実行する前に設定すべき環境変数の設定方法を、実行 OS に応じて表示する。

    仮想環境を有効化 (activate) してから dbt を実行する構成のため、`profiles.yml`
    が参照する環境変数 (`S3_BUCKET_ATHENA`) を、現在のシェルに直接 export する。
    値は `.env` にも保存済みなので、ターミナルを開き直した際はそこから確認できる。
    """
    print_step("dbt コマンドを実行する前に: 環境変数を設定してください")
    print(
        f"以下のコマンドを実行すると、環境変数 {env_key} が設定され、以降は\n"
        "`dbt ...` を実行できます（ターミナルを開き直した場合は再度実行してください）。\n"
    )
    if os.name == "nt":  # Windows
        print("  PowerShell の場合:")
        print(f'    $env:{env_key} = "{env_value}"')
        print("  コマンドプロンプト (cmd.exe) の場合:")
        print(f"    set {env_key}={env_value}")
    else:  # macOS / Linux
        print("  bash / zsh の場合:")
