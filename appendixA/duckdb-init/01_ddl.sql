...
第4章の duckdb-init/01_ddl.sql と同一内容（v1 版の DDL が両者で完全一致しているため）。
...
外部キーを外しているのは、DuckDB が外部キーで参照されている親行を UPDATE できない
（値を変えない列の更新でも Constraint Error になる）ためで、第4章の DuckDB 版と同じ判断。
